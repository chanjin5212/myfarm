...
export default function ProductInquiry({ productId }: ProductInquiryProps) {
  ...
  // 비밀글 확인 로직
  const canViewInquiry = (inquiry: Inquiry) => {
    // 비밀글이 아니면 누구나 볼 수 있음
        ...
        <div className="border-t border-gray-200">
          {inquiries.map((inquiry) => (
            <div key={inquiry.id} className="border-b border-gray-200">
              <div 
                className={`py-4 px-2 ${canViewInquiry(inquiry) ? 'cursor-pointer' : 'cursor-pointer'}`}
                      ...
                      {inquiry.replies && inquiry.replies.length > 0 ? (
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          {inquiry.replies.map((reply) => (
                            <div key={reply.id} className="bg-white p-3 rounded-md mt-2">
