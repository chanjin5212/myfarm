...
export async function GET(
  ...
  try {
    ...
    if (inquiryError) {
      console.error('문의 상세 정보 조회 오류:', inquiryError);
      return NextResponse.json({ error: '문의 정보를 불러오는데 실패했습니다.' }, { status: 500 });
    }
    ...
    const { data: replies, error: repliesError } = await supabase
      ...
      .select(`
        *,
        users:user_id (name, email, nickname, is_admin)
      `)
      .eq('inquiry_id', inquiryId)
      .order('created_at', { ascending: true });

    if (repliesError) {
      console.error('답변 목록 조회 오류:', repliesError);
      return NextResponse.json({ error: '답변 목록을 불러오는데 실패했습니다.' }, { status: 500 });
    }
    ...
    const formattedInquiry = {
      ...
      replies: replies.map(reply => ({
        ...
        user_name: reply.users?.nickname || reply.users?.name || '알 수 없음',
        is_admin: reply.users?.is_admin || false
      }))
    };

    return NextResponse.json(formattedInquiry);
  } catch (error) {
    console.error('문의 상세 정보 API 오류:', error);
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    );
